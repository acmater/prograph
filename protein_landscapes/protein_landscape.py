...
class Protein_Landscape(Landscape):
    ...
    def __init__(self,data=None,
        ...
        self.seed = self.seed_prot.rep

        setattr(self.seed_prot,"tokenized",tuple(self.tokenize(self.seed_prot.rep)))

        self.seq_len     = len(self.seed_prot.rep)
        ...
        self.sequence_mutation_locations = self.boolean_mutant_array(self.seed_prot.rep)
    ...
    def __repr__(self):
        # TODO Finish this
        return f"""Protein_Landscape(seed_seq='{self.seed_prot.rep}',
    ...
    def indexing(self,reference_seq=None,distances=None,positions=None,percentage=None,Bool="or",complement=False):
        ...
        if reference_seq is None:
            reference_seq   = self.seed_prot.rep
            d_data          = self.d_data
        else:
    ...
    def calc_mutated_positions(self):
        ...
        """
        mutated_bools = np.invert(np.all(self.tokenized[:,:-1] == self.tokenize(self.seed_prot.rep),axis=0)) # Calculates the indices all of arrays which are modified.
    ...
    def coloured_seed_string(self):
        ...
        for i,char in enumerate(self.seed_prot.rep):
            if i in idxs:
    ...
    def load(self,name):
        print(f"Loading landscape {name}")
        super().load(name)
